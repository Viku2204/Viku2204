...
@web_app.route("/inventory")
def inventory():
    return render_template('inventory.py')
...
@web_app.route("/register-form", methods=['POST'])
def register_vet():
    ...
    return render_template('success.html', email=session['vet_email'])
...
@web_app.route("/login-form", methods=['POST'])
def login_vet():
    ...
    if len(documents) == 1:
        ...
        print(vars(session))
        return render_template('success.html', email=session['vet_email'], name=session['vet_name'])
    else:
...
def delete_consultation(id):
    ...
    consultation = db.fetch(query)[0]
    db.delete(query)
    return render_template('success.html', message="Consultation Deleted")
...
def export_to_csv():
    ...
    # Add route data to the 'data' list
    data.append(["Route", "Method", "Function Name"])
    for rule in web_app.url_map.iter_rules():
        if rule.endpoint != 'static':
            methods = ','.join(rule.methods)
            function_name = web_app.view_functions[rule.endpoint].__name__
            data.append([rule, methods, function_name])

    # Write data to a CSV file
    with open("app_routes.csv", "w", newline="") as csvfile:
